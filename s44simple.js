...
const S44_RUNTIME_STORE='s44_runtime_draws_v1';
...
function s44GetHistory(){
 let arr=null,source='页面历史';
 try{
  const raw=localStorage.getItem(S44_RUNTIME_STORE);
  if(raw){const x=JSON.parse(raw);if(Array.isArray(x)&&x.length){arr=x;source='导入完整历史'}}
 }catch(e){console.warn('S4.4 runtime history read failed',e)}
 if(!arr){if(typeof draws==='undefined'||!Array.isArray(draws))return {hist:[],source};arr=draws}
 const hist=arr.map(d=>({period:Number(d.period),regular:(d.regular||[]).map(Number),special:Number(d.special)}))
   .filter(d=>Number.isInteger(d.period)&&d.period>0&&d.regular.length===6&&Number.isFinite(d.special))
   .map(d=>d.period===245?Object.assign({},d,{regular:[22,23,14,2,13,38],special:18}):d)
...
function s44sRender(){
 try{
  if(typeof predict!=='function'||typeof s43Compute!=='function')return;
  const hp=s44GetHistory(),hist=hp.hist;if(!hist.length)return;
  const last=hist[hist.length-1],latest=last.period,target=latest+1;
...
setTimeout(s44sRender,1100);let s44ssig='';setInterval(()=>{try{const hp=s44GetHistory(),h=hp.hist;if(!h.length)return;const d=h[h.length-1],raw=localStorage.getItem(S44_RUNTIME_STORE)||'',x=`${raw.length}:${h.length}:${d.period}:${d.special}`;if(x!==s44ssig){s44ssig=x;s44sRender()}}catch(e){console.error(e)}},2200);
